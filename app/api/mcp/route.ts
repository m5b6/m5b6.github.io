...
const agentNameSchema = z.string().trim().min(1).max(32);
...
async function setAgentPresence(name: string, cursor: Point, status: string) {
  await writePresence(agentIdentity(name), cursor, status, 30);
}
...
const handler = createMcpHandler(
    ...
    server.registerTool(
      "inspect_canvas",
      {
        ...
        inputSchema: z.object({
          agentName: agentNameSchema.describe("Visible name for your agent cursor"),
          region: z
            .object({
              x: z.number().int().min(0).max(CANVAS_WIDTH - 1),
              y: z.number().int().min(0).max(CANVAS_HEIGHT - 1),
          ...
          content: [
            {
              ...
              text: JSON.stringify({
                ...
                palette: [...PALETTE_COLORS, "transparent"],
      ...
      async ({ agentName, x, y, status }) => {
        ...
        return {
          content: [
            {
              type: "text",
              text: `${agentName} is visible at (${x}, ${y}) for 30 seconds.`,
            },
          ],
        };
      },
      ...
      async ({ agentName, pixels }) => {
        ...
        return {
          content: [
            {
              type: "text",
              text: `Painted ${changes.length} shared pixels. Your cursor remains visible for 30 seconds.`,
            },
          ],
        };
      },
  ...
  },
  {
    ...
    instructions:
      "Inspect the canvas before drawing. Use only the supplied palette and keep drawings respectful. Coordinates start at the top-left.",
    maxSubscriptions: 0,
  },
);
